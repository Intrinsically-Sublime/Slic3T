...
use Slic3T;
use Slic3T::Geometry qw(epsilon);

{
    my $path = Slic3T::ExtrusionPath->new(polyline => Slic3T::Polyline->new(
    ...
    my $collection = Slic3T::ExtrusionPath::Collection->new(paths => [$path]);
    $collection->detect_arcs(30);
    
    is scalar(@{$collection->paths}), 3, 'path collection now contains three paths';
    isa_ok $collection->paths->[1], 'Slic3T::ExtrusionPath::Arc', 'second one';
}

#==========================================================

{
    my $path1 = Slic3T::ExtrusionPath->new(polyline => Slic3T::Polyline->new(
    ...
    my $path2 = Slic3T::ExtrusionPath->new(
        polyline    => Slic3T::Polyline->new(reverse @{$path1->points}),
        role        => 'fill',
    );
    
    my $collection1 = Slic3T::ExtrusionPath::Collection->new(paths => [$path1]);
    my $collection2 = Slic3T::ExtrusionPath::Collection->new(paths => [$path2]);
    ...
    isa_ok $collection1->paths->[0], 'Slic3T::ExtrusionPath::Arc', 'path';
    isa_ok $collection2->paths->[0], 'Slic3T::ExtrusionPath::Arc', 'path';
