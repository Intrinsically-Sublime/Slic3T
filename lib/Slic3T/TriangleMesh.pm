package Slic3T::TriangleMesh;
use Moo;

use Slic3T::Geometry qw(X Y Z A B unscale same_point);
...
sub clean {
    ...
    Slic3T::debugf "%d narrow facets removed\n", scalar(@narrow_facets_indexes)
...
sub make_loops {
    ...
    foreach my $point_id (grep $a_count{$_} > 1, keys %a_count) {
        ...
        Slic3T::debugf "%d lines start at point %d\n", scalar(@lines_starting_here), $point_id;
        ...
        if (@lines_starting_here == 2 && join(',', sort map $_->facet_edge, @lines_starting_here) eq 'bottom,top') {
            ...
            Slic3T::debugf "  this shouldn't happen and should be further investigated\n";
            if (0) {
                require "Slic3T/SVG.pm";
                Slic3T::SVG::output(undef, "same_point.svg",
    ...
    CYCLE: for (my $i = 0; $i <= $#lines; $i++) {
        ...
        push @polygons, Slic3T::Polygon->new(@points);
        Slic3T::debugf "  Discovered %s polygon of %d points\n",
...
sub rotate {
    ...
    my $rad = Slic3T::Geometry::deg2rad($deg);
    
    # transform vertex coordinates
    foreach my $vertex (@{$self->vertices}) {
        @$vertex = (@{ +(Slic3T::Geometry::rotate_points($rad, undef, [ $vertex->[X], $vertex->[Y] ]))[0] }, $vertex->[Z]);
    }
}
...
sub slice_facet {
    ...
    Slic3T::debugf "z: min = %.0f, max = %.0f\n", $min_z, $max_z;
    
    if ($max_z == $min_z) {
        Slic3T::debugf "Facet is horizontal; ignoring\n";
        return;
    }
    
    # calculate the layer extents
    my $first_layer_height = $Slic3T::layer_height * $Slic3T::first_layer_height_ratio;
    my $min_layer = int((unscale($min_z) - ($first_layer_height + $Slic3T::layer_height / 2)) / $Slic3T::layer_height) - 2;
    $min_layer = 0 if $min_layer < 0;
    my $max_layer = int((unscale($max_z) - ($first_layer_height + $Slic3T::layer_height / 2)) / $Slic3T::layer_height) + 2;
    Slic3T::debugf "layers: min = %s, max = %s\n", $min_layer, $max_layer;
...
sub intersect_facet {
    ...
    for my $e (0..2) {
        ...
        if ($a->[Z] == $b->[Z] && $a->[Z] == $z) {
            ...
            push @lines, Slic3T::TriangleMesh::IntersectionLine->new(
                a           => [$a->[X], $a->[Y]],
                b           => [$b->[X], $b->[Y]],
                a_id        => $a_id,
                b_id        => $b_id,
    ...
    if (@points) {
        ...
        return Slic3T::TriangleMesh::IntersectionLine->new(
