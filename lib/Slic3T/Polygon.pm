package Slic3T::Polygon;
use strict;
use warnings;

# a polygon is a closed polyline.
use parent 'Slic3T::Polyline';

use Slic3T::Geometry qw(polygon_lines polygon_remove_parallel_continuous_edges
    scale polygon_remove_acute_vertices polygon_segment_having_point point_in_polygon);
use Slic3T::Geometry::Clipper qw(JT_MITER);
...
sub area {
    my $self = shift;
    return Slic3T::Geometry::Clipper::area($self);
}

sub safety_offset {
    my $self = shift;
    return (ref $self)->new(Slic3T::Geometry::Clipper::safety_offset([$self])->[0]);
}
...
sub subdivide {
    ...
    for (my $i = 0; $i <= $#$self; $i++) {
        my $len = Slic3T::Geometry::line_length([ $self->[$i-1], $self->[$i] ]);
        ...
        my @new_points = map Slic3T::Point->new($_),
            map Slic3T::Geometry::point_along_segment($self->[$i-1], $self->[$i], $spacing * $_),
            1..$num_points;
...
sub is_printable {
    ...
    my $offsets = Math::Clipper::offset([$p], -(scale $Slic3T::flow_spacing / 2), $Slic3T::scaling_factor * 100000, JT_MITER, 2);
    return @$offsets ? 1 : 0;
}
