...
use Slic3T::Geometry qw(X Y A B point_in_polygon same_line);
use Slic3T::Geometry::Clipper qw(union_ex JT_MITER);
...
sub new {
    ...
    if (@_ == 1 && ref $_[0] eq 'HASH') {
        $self = [
            Slic3T::Polygon->new($_[0]{outer}),
            map Slic3T::Polygon->new($_), @{$_[0]{holes}},
        ];
    } else {
        $self = [ map Slic3T::Polygon->new($_), @_ ];
    }
    bless $self, $class;
    $self;
}
...
sub offset {
    ...
    $scale      ||= $Slic3T::scaling_factor * 1000000;
...
sub safety_offset {
    ...
    return (ref $self)->new(
        ...
        @{ Slic3T::Geometry::Clipper::safety_offset([$self->holes]) },
    );
}
...
sub bounding_box {
    my $self = shift;
    return Slic3T::Geometry::bounding_box($self->contour);
}

sub bounding_box_polygon {
    my $self = shift;
    my @bb = $self->bounding_box;
    return Slic3T::Polygon->new([
        [ $bb[0], $bb[1] ],
        [ $bb[2], $bb[1] ],
        [ $bb[2], $bb[3] ],
        [ $bb[0], $bb[3] ],
...
sub clip_line {
    my $self = shift;
    my ($line) = @_;  # line must be a Slic3T::Line object
...
sub medial_axis {
    ...
    foreach my $polygon (@$expolygon) {
        Slic3T::Geometry::polyline_remove_short_segments($polygon, $width / 2);
    ...
    # cleanup
    Slic3T::Geometry::polyline_remove_short_segments($polyline, $width / 2);
    $polyline = Slic3T::Geometry::douglas_peucker($polyline, $width / 7);
    Slic3T::Geometry::polyline_remove_parallel_continuous_edges($polyline);
    
    if (Slic3T::Geometry::same_point($polyline->[0], $polyline->[-1])) {
        return undef if @$polyline == 2;
        return Slic3T::Polygon->new(@$polyline[0..$#$polyline-1]);
    } else {
        return Slic3T::Polyline->new($polyline);
    }
}
