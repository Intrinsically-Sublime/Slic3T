package Slic3T::Layer;
use Moo;

use Math::Clipper ':all';
use Slic3T::Geometry qw(scale collinear X Y A B PI rad2deg_dir bounding_box_center);
use Slic3T::Geometry::Clipper qw(union_ex diff_ex intersection_ex xor_ex is_counter_clockwise);
...
has 'lines' => (
    is      => 'rw',
    #isa     => 'ArrayRef[Slic3T::TriangleMesh::IntersectionLine]',
    default => sub { [] },
);
...
has 'perimeters' => (
    is      => 'rw',
    #isa     => 'ArrayRef[Slic3T::ExtrusionLoop]',
    default => sub { [] },
);
...
has 'support_fills' => (
    is      => 'rw',
    #isa     => 'Slic3T::ExtrusionPath::Collection',
);

# ordered collection of extrusion paths to fill surfaces
has 'fills' => (
    is      => 'rw',
    #isa     => 'ArrayRef[Slic3T::ExtrusionPath::Collection]',
    default => sub { [] },
);
...
sub slice_z {
    my $self = shift;
    if ($self->id == 0) {
        return ($Slic3T::layer_height * $Slic3T::first_layer_height_ratio) / 2 / $Slic3T::scaling_factor;
    }
    return (($Slic3T::layer_height * $Slic3T::first_layer_height_ratio)
        + (($self->id-1) * $Slic3T::layer_height)
        + ($Slic3T::layer_height/2)) / $Slic3T::scaling_factor;
}
...
sub print_z {
    my $self = shift;
    return (($Slic3T::layer_height * $Slic3T::first_layer_height_ratio)
        + ($self->id * $Slic3T::layer_height)) / $Slic3T::scaling_factor;
}

sub height {
    my $self = shift;
    return $self->id == 0
        ? ($Slic3T::layer_height * $Slic3T::first_layer_height_ratio)
        : $Slic3T::layer_height;
}
...
sub make_surfaces {
    ...
    
    {
        ...
        Slic3T::debugf "  %d surface(s) having %d holes detected from %d polylines\n",
            scalar(@$expolygons), scalar(map $_->holes, @$expolygons), scalar(@$loops);
        
        push @{$self->slices},
            map Slic3T::Surface->new(expolygon => $_, surface_type => 'internal'),
                @$expolygons;
    }
    
    # the contours must be offsetted by half extrusion width inwards
    {
        my $distance = scale $Slic3T::flow_width / 2;
        my @surfaces = @{$self->slices};
        @{$self->slices} = ();
        foreach my $surface (@surfaces) {
            push @{$self->slices}, map Slic3T::Surface->new
        ...
        Slic3T::debugf "  %d thin walls detected\n", scalar(@{$self->thin_walls}) if @{$self->thin_walls};
    }
    
    if (0) {
        require "Slic3T/SVG.pm";
        Slic3T::SVG::output(undef, "surfaces.svg",
...
sub prepare_fill_surfaces {
    ...
    {
        my $min_area = ((7 * $Slic3T::flow_spacing / $Slic3T::scaling_factor)**2) * PI;
        my $small_internal = [
        ...
        my @top = map Slic3T::Surface->new(expolygon => $_, surface_type => 'top'), @$union;
...
sub remove_small_surfaces {
    ...
    foreach my $surface (@surfaces) {
        ...
        push @{$self->fill_surfaces}, map Slic3T::Surface->new(
            expolygon => $_,
            surface_type => $surface->surface_type), @offsets;
    }
    
    Slic3T::debugf "identified %d small surfaces at layer %d\n",
...
sub remove_small_perimeters {
    ...
    Slic3T::debugf "removed %d unprintable perimeters at layer %d\n",
...
sub process_bridges {
    ...
    return if $Slic3T::fill_density == 0;
    ...
    SURFACE: foreach my $surface (@solid_surfaces) {
        ...
        my ($contour_offset) = $expolygon->contour->offset(scale $Slic3T::flow_spacing * sqrt(2));
        ...
        if (0) {
            require "Slic3T/SVG.pm";
            Slic3T::SVG::output(undef, "bridge_surfaces.svg",
                green_polygons  => [ map $_->p, @supporting_surfaces ],
                red_polygons    => [ @$expolygon ],
            );
        }
        
        Slic3T::debugf "Found $description on layer %d with %d support(s)\n", 
        ...
        if ($surface->surface_type eq 'bottom') {
            ...
            Slic3T::debugf "  Bridge is supported on %d edge(s)\n", scalar(@edges);
            Slic3T::debugf "  and covers a hole\n" if $bridge_over_hole;
            
            if (0) {
                require "Slic3T/SVG.pm";
                Slic3T::SVG::output(undef, "bridge_edges.svg",
                    polylines       => [ map $_->p, @edges ],
                );
            }
            
            if (@edges == 2) {
                my @chords = map Slic3T::Line->new($_->[0], $_->[-1]), @edges;
                my @midpoints = map $_->midpoint, @chords;
                my $line_between_midpoints = Slic3T::Line->new(@midpoints);
                ...
                if (@{$edges[0]} > 2) {
                    my $line = Slic3T::Line->new($edges[0]->[0], $edges[0]->[-1]);
                ...
                foreach my $point (map @$, @edges) {
                    my $line = Slic3T::Line->new($center, $point);
            ...
            Slic3T::debugf "  Optimal infill angle of bridge on layer %d is %d degrees\n",
        ...
        # now, extend our bridge by taking a portion of supporting surfaces
        {
            ...
            push @bridges, map Slic3T::Surface->new(
                expolygon => $_,
    ...
    # now we need to merge bridges to avoid overlapping
    {
        ...
        my @surface_groups = Slic3T::Surface->group(@bridges);
    ...
    # apply bridges to layer
    {
        ...
        foreach my $group (Slic3T::Surface->group(@surfaces)) {
