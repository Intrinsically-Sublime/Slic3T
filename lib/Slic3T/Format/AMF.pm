package Slic3T::Format::AMF;
use Moo;

use Slic3T::Geometry qw(X Y Z);

sub read_file {
    my $self = shift;
    my ($file) = @_;
    
    eval "require Slic3T::Format::AMF::Parser; 1"
    ...
    XML::SAX::PurePerl
        ->new(Handler => Slic3T::Format::AMF::Parser->new(
            _vertices           => $vertices,
    ...
    $_ = Slic3T::TriangleMesh->new(vertices => $vertices, facets => $_)
...
sub write_file {
    ...
    printf $fh qq{  <metadata type="cad">Slic3T %s</metadata>\n}, $Slic3T::VERSION;
