package Slic3T::Format::STL;
use Moo;

use Slic3T::Geometry qw(X Y Z triangle_normal);
...
sub read_file {
            ...
            for (my $v = 0; $v <= $vertices_count; $v++) {
                ...
                while (@{$vertices_facets[$v]}) {
                    ...
                    if ($more_than_one_vertex_in_this_point) {
                        Slic3T::debugf "  more than one vertex in the same point\n";
    ...
    return Slic3T::TriangleMesh->new(vertices => $vertices, facets => $facets);
}
