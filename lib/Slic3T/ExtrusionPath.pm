package Slic3T::ExtrusionPath;
use Moo;

use Slic3T::Geometry qw(PI X Y epsilon deg2rad rotate_points);

# the underlying Slic3T::Polyline objects holds the geometry
has 'polyline' => (
    is          => 'ro',
    required    => 1,
...
sub BUILD {
    my $self = shift;
    bless $self->polyline, 'Slic3T::Polyline';
}
...
sub clip_end {
    ...
    while ($distance > 0) {
        ...
        my $new_point = Slic3T::Geometry::point_along_segment($last_point, $self->points->[-1], $distance);
        push @{$self->points}, Slic3T::Point->new($new_point);
        $distance = 0;
    }
}
...
sub split_at_acute_angles {
    ...
    my $angle_limit = abs(Slic3T::Geometry::deg2rad(40));
    my @points = @{$self->p};
    ...
    while (my $p3 = shift @points) {
        my $angle = abs(Slic3T::Geometry::angle3points($p[-1], $p[-2], $p3));
        ...
        if ($angle < $angle_limit) {
            ...
            push @paths, (ref $self)->new(
                polyline        => Slic3T::Polyline->new(\@p),
                role            => $self->role,
                depth_layers    => $self->depth_layers,
             );
            @p = ($p3);
...
sub detect_arcs {
    ...
    $len_epsilon ||= 10 / $Slic3T::scaling_factor;
    ...
    CYCLE: while (@points >= 4) {
        POINT: for (my $i = 0; $i <= $#points - 3; $i++) {
            my $s1 = Slic3T::Line->new($points[$i],   $points[$i+1]);
            my $s2 = Slic3T::Line->new($points[$i+1], $points[$i+2]);
            my $s3 = Slic3T::Line->new($points[$i+2], $points[$i+3]);
            my $s1_len = $s1->length;
            my $s2_len = $s2->length;
            my $s3_len = $s3->length;
            ...
            next if abs($s1s2_angle - $s2s3_angle) > $Slic3T::Geometry::parallel_degrees_limit;
            next if abs($s1s2_angle) < $Slic3T::Geometry::parallel_degrees_limit;     # ignore parallel lines
            ...
            for (my $j = $i+3; $j < $#points; $j++) {
                my $line = Slic3T::Line->new($points[$j], $points[$j+1]);
                ...
                last if abs($s1s2_angle - $anglediff) > $Slic3T::Geometry::parallel_degrees_limit;
            ...
            my $arc_center;
            {
                ...
                my $last_mid = Slic3T::Line->new($points[$last_j-1], $points[$last_j])->midpoint;
                my $rotation_angle = PI/2 * ($orientation eq 'ccw' ? -1 : 1);
                my $ray1     = Slic3T::Line->new($s1_mid,   rotate_points($rotation_angle, $s1_mid,   $points[$i+1]));
                my $last_ray = Slic3T::Line->new($last_mid, rotate_points($rotation_angle, $last_mid, $points[$last_j]));
                $arc_center = $ray1->intersection($last_ray, 0) or next POINT;
            }
            
            my $arc = Slic3T::ExtrusionPath::Arc->new(
                polyline    => Slic3T::Polyline->new(\@arc_points),
                role        => $self->role,
            ...
            push @paths, (ref $self)->new(
                polyline        => Slic3T::Polyline->new(@points[0..$i]),
                role            => $self->role,
                depth_layers    => $self->depth_layers,
            ) if $i > 0;
    ...
    push @paths, (ref $self)->new(
        polyline        => Slic3T::Polyline->new(\@points),
        role            => $self->role,
        depth_layers    => $self->depth_layers,
    ) if @points > 1;
