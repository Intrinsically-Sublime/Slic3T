package Slic3T::Perimeter;
use Moo;

use Math::Clipper ':all';
use Slic3T::Geometry qw(X Y PI shortest_path scale unscale);
use Slic3T::Geometry::Clipper qw(diff_ex);

sub make_perimeter {
    my $self = shift;
    my ($layer) = @_;
    Slic3T::debugf "Making perimeters for layer %d\n", $layer->id;
    ...
    # @perimeters = (
    #    [ # first island
    #        [ Slic3T::ExPolygon, Slic3T::ExPolygon... ],  #depth 0: outer loop
    #        [ Slic3T::ExPolygon, Slic3T::ExPolygon... ],  #depth 1: inner loop
    #    ],
    #    [ # second island
    #        ...
    #    ]
    ...
    foreach my $surface (@surfaces) {
        ...
        foreach my $hole ($last_offsets[0]->holes) {
            ...
            next unless $area <= $Slic3T::small_perimeter_area;
            my $radius = sqrt($area / PI);
            my $new_radius = (scale($Slic3T::flow_width) + sqrt((scale($Slic3T::flow_width)**2) + (4*($radius**2)))) / 2;
            @$hole = map Slic3T::Point->new($_), @{ +($hole->offset(+ ($new_radius - $radius)))[0] };
        }
        
        # create other offsets
        push @perimeters, [];
        for (my $loop = 0; $loop < $Slic3T::perimeters; $loop++) {
            ...
            $distance = scale $Slic3T::flow_spacing;
        }
        ...
        # create one more offset to be used as boundary for fill
        {
            ...
            push @{ $layer->thin_fills },
                grep $_,
                map $_->medial_axis(scale $Slic3T::flow_width),
                @$small_gaps if 0;
        }
    }
    ...
    foreach my $island (@perimeters) {
        ...
        foreach my $hole (@holes) {
            push @{ $layer->perimeters }, Slic3T::ExtrusionLoop->new(polygon => $hole, role => 'perimeter');
        }
        
        # do contours starting from innermost one
        foreach my $contour (map $_->contour, map @$_, reverse @$island) {
            push @{ $layer->perimeters }, Slic3T::ExtrusionLoop->new(polygon => $contour, role => 'perimeter');
        }
    }
    
    # detect small perimeters by checking their area
    for (@{ $layer->perimeters }) {
        $_->role('small-perimeter') if abs($_->polygon->area) < $Slic3T::small_perimeter_area;
    }
    
    # add thin walls as perimeters
    for (@{ $layer->thin_walls }) {
        if ($_->isa('Slic3T::Polygon')) {
            push @{ $layer->perimeters }, Slic3T::ExtrusionLoop->new(polygon => $_, role => 'perimeter');
        } else {
            push @{ $layer->perimeters }, Slic3T::ExtrusionPath->new(polyline => $_, role => 'perimeter');
        }
    }
}
