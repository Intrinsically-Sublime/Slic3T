package Slic3T::Fill::Concentric;
use Moo;

extends 'Slic3T::Fill::Base';

use Slic3T::Geometry qw(scale unscale X1 Y1 X2 Y2);
...
sub fill_surface {
    ...
    # make paths
    my @paths = ();
    my $cur_pos = Slic3T::Point->new(
        ($bounding_box->[X1] + $bounding_box->[X2]) / 2,
        ($bounding_box->[Y1] + $bounding_box->[Y2]) / 2,
    );
    foreach my $loop (map Slic3T::ExtrusionLoop->new(polygon => $_, role => 'fill'), @loops) {
        ...
        $path->clip_end(scale $Slic3T::flow_width * 0.15);
