...
sub infill_direction {
    ...
    # set infill angle
    my (@rotate, @shift);
    $rotate[0] = Slic3T::Geometry::deg2rad($Slic3T::fill_angle);
    ...
    if ($self->layer) {
        ...
        $rotate[0] = Slic3T::Geometry::deg2rad($Slic3T::fill_angle) + $angle if $angle;
    }
        
    # use bridge angle
    if (defined $surface->bridge_angle) {
        Slic3T::debugf "Filling bridge with angle %d\n", $surface->bridge_angle;
        $rotate[0] = Slic3T::Geometry::deg2rad($surface->bridge_angle);
    }
    
    @shift = @{ +(Slic3T::Geometry::rotate_points(@rotate, \@shift))[0] };
    return [\@rotate, \@shift];
}
...
sub rotate_points_back {
    ...
    if ($rotate[0]) {
        @$paths = map [ Slic3T::Geometry::rotate_points(-$rotate[0], $rotate[1], @$_) ], 
            map [ Slic3T::Geometry::move_points([map -$_, @shift], @$_) ], @$paths;
    }
}
