package Slic3T::Fill::Rectilinear;
use Moo;

extends 'Slic3T::Fill::Base';

use Slic3T::Geometry qw(X1 Y1 X2 Y2 A B X Y scale unscale epsilon);
...
sub fill_surface {
    ...
    my $is_line_pattern = $self->isa('Slic3T::Fill::Line');
    my @vertical_lines = ();
    for (my $i = 0; $x <= $bounding_box->[X2] + scale epsilon; $i++) {
        my $vertical_line = Slic3T::Line->new([$x, $bounding_box->[Y2]], [$x, $bounding_box->[Y1]]);
    ...
    # connect lines
    {
        my $collection = Slic3T::ExtrusionPath::Collection->new(
            paths => [ map Slic3T::ExtrusionPath->new(polyline => Slic3T::Polyline->new(@$_), role => 'bogus'), @paths ],
        );
        @paths = ();
        ...
        foreach my $path ($collection->shortest_path) {
            if (@paths) {
                ...
                if ($can_connect->(@distance, $paths[-1][-1], $path->points->[0])
                    && $expolygon_off->encloses_line(Slic3T::Line->new($paths[-1][-1], $path->points->[0]))) {
