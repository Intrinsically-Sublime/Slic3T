...
sub scale   ($) { $_[0] / $Slic3T::scaling_factor }
sub unscale ($) { $_[0] * $Slic3T::scaling_factor }
...
sub polyline_lines {
    ...
    foreach my $point (@$polygon) {
        push @lines, Slic3T::Line->new($last_point, $point) if $last_point;
        $last_point = $point;
    }
...
sub move_points {
    my ($shift, @points) = @_;
    return map Slic3T::Point->new($shift->[X] + $_->[X], $shift->[Y] + $_->[Y]), @points;
}
...
sub merge_collinear_lines {
    ...
    for (my $i = 0; $i <= $#$lines-1; $i++) {
        for (my $j = $i+1; $j <= $#$lines; $j++) {
            ...
            my $new_line = Slic3T::Line->new([$x[0], $y[0]], [$x[-1], $y[-1]]);
            for (X, Y) {
    ...
    Slic3T::debugf "  merging %d lines resulted in %d lines\n", $line_count, scalar(@$lines);
...
sub _line_intersection {
  ...
  return [Slic3T::Point->new($x, $y), $h10 >= 0 && $h10 <= 1 && $h32 >= 0 && $h32 <= 1];
}
...
sub polyline_remove_parallel_continuous_edges {
    ...
    for (my $i = $isPolygon ? 0 : 2; $i <= $#$points && @$points >= 3; $i++) {
        if (Slic3T::Geometry::lines_parallel([$points->[$i-2], $points->[$i-1]], [$points->[$i-1], $points->[$i]])) {
